...
export const articleService = {
  ...
  getAllArticles: async (params = {}): Promise<Article[]> => {
    try {
      ...
      const response = await fetch(`${API_URL}/api/articles?${queryString}`, {
        credentials: 'include',
      });
  ...
  getTrendingArticles: async (limit = 5): Promise<Article[]> => {
    try {
      ...
      const response = await fetch(`${API_URL}/api/articles/trending?limit=${limit}`, {
        credentials: 'include',
      });
  ...
  getArticlesByCategory: async (category: string): Promise<Article[]> => {
    try {
      ...
      const response = await fetch(`${API_URL}/api/articles/category/${encodeURIComponent(category)}`, {
        credentials: 'include',
      });
  ...
  getRelatedArticles: async (slug: string, limit = 3): Promise<Article[]> => {
    try {
      ...
      const response = await fetch(`${API_URL}/api/articles/${slug}/related?limit=${limit}`, {
        credentials: 'include',
      });
  ...
  incrementViewCount: async (articleId: string): Promise<void> => {
    try {
      ...
      const response = await fetch(`${API_URL}/api/articles/${articleId}/view`, {
        method: 'POST',
        credentials: 'include',
      });
  ...
  searchArticles: async (query: string): Promise<Article[]> => {
    try {
      ...
      const response = await fetch(`${API_URL}/api/articles?search=${encodeURIComponent(query)}`, {
        credentials: 'include',
      });
  ...
  addComment: async (articleId: string, commentData: { content: string }): Promise<{ success: boolean; data?: any; error?: string }> => {
    try {
      ...
      const response = await fetch(`${API_URL}/api/articles/${articleId}/comment`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  getAuthors: async (): Promise<{ success: boolean; data?: any[]; error?: string }> => {
    try {
      ...
      const response = await fetch(`${API_URL}/api/users/authors`, {
        credentials: 'include',
      });
  ...
  changeArticleAuthor: async (articleId: string, newAuthorId: string): Promise<{ success: boolean; data?: any; error?: string }> => {
    try {
      ...
      const response = await fetch(`${API_URL}/api/articles/${articleId}/author`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
